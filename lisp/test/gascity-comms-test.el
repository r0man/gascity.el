    ...
    (let (shown opened sid)
      (cl-letf (((symbol-function 'gascity-bead-show)
                ...
                ((symbol-function 'gascity-events--open-agent)
                 (lambda (agent) (setq opened agent)))
                ...
                ((symbol-function 'gascity-store-fetch)
                 (lambda (args cb &rest _)
                   ...
                   (funcall cb `((sessions . [((id . ,sid)
                                               ...
                                               (session_name . "gc__run-operator-1")
                                               (work_dir . "/w/op"))]))))))
        (cl-flet ((goto (pred)
                    (goto-char (point-min))
          ...
          (gascity-events-visit)
          (should (equal (gascity-agent-name opened) "beads.el/gc.run-operator-1"))
          (should (equal (gascity-agent-work-dir opened) "/w/op"))
          (should (equal (gascity-agent-session-name opened) "gc__run-operator-1"))
